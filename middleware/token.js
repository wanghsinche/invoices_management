...
let noncedb = new sqlite3.Database(':memory:');
// 其实客户端传过来不就好了么。。。
// 反正只是防replay，我这里就不处理了

noncedb.run('CREATE TABLE "nonce" ( `stamp` INTEGER, `nonce` INTEGER, `lastnum` INTEGER DEFAULT 0 )');


// every 60 minutes
setTimeout(function clearNoncedb(period) {
    let expire = Date.now() - period;
    global.logger.info('clearNoncedb');
...
module.exports = function(req, res, next) {
    ...
    new Promise((resolve, reject) => {
                ...
                noncedb.get('SELECT rowid, lastnum FROM nonce WHERE nonce = $nonce', {
                    ...
                    if (err) {
                        ...
                        if (typeof row === 'undefined') {
                            ...
                            global.logger.info('new nonce, write into nonce database');
                            ...
                            // old nonce, new time
                            // update
                            global.logger.info('old nonce, new time, update');
                            global.logger.info(row.lastnum, Number(reqtimes));
                            ...
                            global.logger.info('old nonce, bad time, abandon nonce');
            ...
            // give userid
            global.logger.info('authorized successfully');
            next();
        })
        .catch(function(code) {
            if (code === 0) {
